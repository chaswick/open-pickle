...
public interface MatchRepository extends JpaRepository<Match, Long> {
        ...
        List<Match> findRecentPlayedMatchesForPlayers(@Param("players") Collection<User> players,
        ...
        @Query("select m from Match m " +
                        ...
                        "and (m.state is null or m.state <> com.w3llspring.fhpb.web.model.MatchState.NULLIFIED) " +
                        "order by m.playedAt desc, m.id desc")
        List<Match> findRecentPlayedMatchesForPlayersInSeason(@Param("season") LadderSeason season,
        ...
        @Query("select m from Match m " +
                        ...
                        "and (m.state is null or m.state <> com.w3llspring.fhpb.web.model.MatchState.NULLIFIED) " +
                        "order by m.playedAt desc, m.id desc")
        List<Match> findRecentPlayedMatchesForPlayersInSession(@Param("sessionConfigId") Long sessionConfigId,
                        @Param("players") Collection<User> players,
                        @Param("start") Instant start,
                        @Param("end") Instant end);

        // Logging-time/diagnostic query: this intentionally uses createdAt, not playedAt.
        List<Match> findByCreatedAtGreaterThanEqualAndCreatedAtLessThan(Instant startInclusive, Instant endExclusive);
        ...
        List<Match> findByCreatedAtInRange(@Param("start") Instant start,
                        @Param("end") Instant end);
        ...
        @Query("select m from Match m " +
                        "where m.season = :season " +
                        "order by m.playedAt desc, m.id desc")
        List<Match> findBySeasonOrderByPlayedAtDescIncludingNullified(@Param("season") LadderSeason season);
        ...
        long countByLoggedBy_IdAndCreatedAtGreaterThanEqual(Long loggedById, Instant createdAtStartInclusive);
        ...
        @Query("select m.id from Match m " +
                        ...
                        "and (m.state is null or m.state <> com.w3llspring.fhpb.web.model.MatchState.NULLIFIED) " +
                        "order by m.playedAt desc, m.id desc")
        List<Long> findParticipantMatchIdsOrderByPlayedAtDesc(@Param("user") User user, Pageable pageable);

        @Query("select m.id from Match m " +
                        "where (m.a1 = :user or m.a2 = :user or m.b1 = :user or m.b2 = :user) " +
                        "order by m.playedAt desc, m.id desc")
        List<Long> findParticipantMatchIdsOrderByPlayedAtDescIncludingNullified(@Param("user") User user, Pageable pageable);
        ...
        @Query("select m from Match m " +
                        ...
                        "and (m.state is null or m.state <> com.w3llspring.fhpb.web.model.MatchState.NULLIFIED) " +
                        "order by m.playedAt desc, m.id desc")
        Page<Match> findBySeasonOrderByPlayedAtDesc(@Param("season") LadderSeason season, Pageable pageable);

        @Query("select m from Match m " +
                        "where m.season = :season " +
                        "order by m.playedAt desc, m.id desc")
        Page<Match> findBySeasonOrderByPlayedAtDescIncludingNullified(@Param("season") LadderSeason season, Pageable pageable);
        ...
        @Query("select m from Match m " +
                        ...
                        "and (m.state is null or m.state <> com.w3llspring.fhpb.web.model.MatchState.NULLIFIED) " +
                        "order by m.playedAt desc, m.id desc")
        List<Match> findUserCorrectedMatchesByParticipant(@Param("userId") Long userId, 
                        Pageable pageable);

        // Trust system: Find matches logged by a specific user in a season
        @Query("select m from Match m " +
                        "where m.loggedBy.id = :userId " +
                        "and m.season.id = :seasonId")
        List<Match> findByLoggedByIdAndSeasonId(@Param("userId") Long userId, 
                        @Param("seasonId") Long seasonId);

        // Batch fetch matches logged by any of the given users within a season (join fetch player references to avoid N+1)
        @Query("select distinct m from Match m " +
                        "left join fetch m.a1 " +
                        "left join fetch m.a2 " +
                        "left join fetch m.b1 " +
                        ...
                        "and m.season.id = :seasonId")
        List<Match> findByLoggedByIdInAndSeasonId(@Param("userIds") java.util.Collection<Long> userIds,
                        @Param("seasonId") Long seasonId);
        ...
        List<Match> findBySeasonOrSourceSessionTargetSeasonOrderByPlayedAtAsc(
                        ...
                        @Param("targetSeasonId") Long targetSeasonId);
